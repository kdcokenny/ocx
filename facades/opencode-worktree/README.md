...
2. **cmux** - If `cmux` is available and either `CMUX_WORKSPACE_ID` is present, or external socket control is explicitly enabled (`CMUX_SOCKET_PATH` with `CMUX_SOCKET_MODE=allowAll`), uses cmux-native launch flow first.
