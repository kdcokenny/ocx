...
Validation logic is centralized in `packages/cli/src/lib/validators/`. See `validators/AGENTS.md` for detailed architecture.

### Key Constraints

- Validators are pure functions: `input → ValidationResult`
- Validation runs in sequence: schema → files → circular deps → duplicates
- Schema validation failure short-circuits remaining checks
- Both `ocx build` and `ocx validate` use the same validators via `runValidation()`
- `buildRegistry()` fails if `!validationResult.valid`
