...
OCX uses a unified validation architecture that ensures consistent checking across both `ocx build` and `ocx validate` commands. All validation logic is centralized in the `packages/cli/src/lib/validators/` module.
...
**Location:** `packages/cli/src/lib/validators/index.ts`
...
**Location:** `packages/cli/src/lib/validators/run-validation.ts`
...
| File | Purpose |
|------|---------|
| `lib/validators/index.ts` | Individual validator functions |
| `lib/validators/run-validation.ts` | Validation orchestrator |
| `lib/validate-registry-local.ts` | Uses validators via runValidation |
