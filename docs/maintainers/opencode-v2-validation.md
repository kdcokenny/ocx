...
- `bun test tests` in `packages/cli`: 562 passed, zero failures. Six compiled-binary checks ran separately and passed. The remaining skips are network/template-release cases that require an already-published V3 tag or a changed default branch; local template scaffolding and registry building were exercised manually.
...
The final publication review added journaling before candidate population, so dead-process recovery also removes partially populated builds. Subprocess checks cover exits during population and between publication renames. Quiet/JSON registry output is covered for add, list, remove, and dry-run. The complete CLI suite now passes 562 tests. A deliberately hung legacy fixture exited at its 30-second deadline with an explicit timeout message in both the assertion and `command-1.log` (`/tmp/ocx-legacy-install-845cr7`); the normal published V1 probe still passes (`/tmp/ocx-legacy-install-BhfarY`).

## Documentation restoration

The documentation follow-up restores full registry starter and worker guides, maintainer instructions, and the code-review method. The V2 manual now has 20 pages covering installation, quick start, all retained CLI commands, configuration, profiles, registry authoring/protocol, migration, verification, and recovery. All 13 removed Markdown definitions/profile notes are preserved with their complete original bodies under `legacy/docs/`, with V1 notices and source provenance. The existing V1 website reference remains available.

Validation performed for the restoration:

- From `docs/`, `bunx mint validate` passes and `bunx mint broken-links` reports no broken links across the documentation tree. These invoke the Mintlify CLI package, not repository package scripts. Historical testing links and nested template fences were repaired as part of that check.
- A local audit checks 174 relative links, navigation targets, matching scaffold/fixture guides, and exact preservation of the 13 archived Markdown bodies after removing their added notices.
- Playwright opens all 20 V2 routes in the local Mintlify preview. Desktop screenshots of the overview, registry tutorial, and component reference were inspected; four mobile views show no page overflow. Report: `/tmp/ocx-docs-browser-report.json`.
- A manual walkthrough runs 40 built-CLI invocations against temporary XDG directories and a local HTTP registry: profile creation/selection/cloning/rename/removal, custom-template scaffolding, manifest examples, validation/build, project/profile installation, source scopes, update previews, local-edit preservation/conflicts/forced repair, verification, removal, recipe dependencies, and preview/apply import. Fixture: `/tmp/ocx-docs-followthrough-qtcg1egv`.
- Official OpenCode 2.0.3 starts successfully through the documented profile. Its API discovers the guide's reviewer with its edit-denial permission, command, and skill. Observed definitions are saved in the walkthrough fixture; no paid provider request was made.
- Both worker builds pass, and all 61 restored V1 files still match `legacy/manifest.json`. The V2 catalog includes the restored full code-review skill.

The subsequent Cubic pass also identified missing-source error handling, source-root symlink normalization, and unsafe publication-recovery paths. The fixes preserve structured build errors and reject replaced stage/backup/candidate/output paths or symlinked journals before recovery changes files. Eight additional regression cases bring the full CLI suite to **570 passed, 12 skipped, zero failures**. Repository checks/builds pass. Manual built-CLI probes verify missing-source JSON errors, linked-source refusal, and refusal of a symlinked recovery stage while preserving external bytes and the journal (`/tmp/ocx-doc-review-manual-671aruaq`). Review dispositions and the current CI state remain on PR #272.

The documentation review follow-up clarifies receipt-origin updates, pending-recovery deletion guards, receipt format numbering, native routing exclusions, and Yarn's existing self-update option (including CLI help). The fixture now includes the deployment configs and example skill described by its README. The subpath deployment guide includes a persistent post-build discovery rewrite. Manual checks validated that exact script over local HTTP, a complete scaffold from the fixture, the `mv` alias, and refused native administration routes (`/tmp/ocx-doc-clarifications-l4idn8po`). A separate install/update exercise confirms that changing the configured alias does not move updates away from the original receipt URL (`/tmp/ocx-receipt-origin-doc-lauhkiu7`). Mintlify build/link checks and the focused scaffold tests pass again.
