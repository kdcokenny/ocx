...
A plugin for OpenCode that delivers native desktop notifications when tasks complete, errors occur, or the AI needs your input. It prioritizes native notifier delivery on macOS, Windows, and Linux, with an additional cmux-native path when available.
...
When OpenCode runs inside cmux (with `CMUX_WORKSPACE_ID` set), the plugin prefers native cmux notifications via:
